...
export default function RadialOrbitalTimeline({
  ...
  const toggleItem = (id: number) => {
    setExpandedItems((prev) => {
      ...
      if (!prev[id]) {
        const selectedItem = timelineData.find((item) => item.id === id) ?? null;
        setActiveNodeId(id);
        setAutoRotate(false);
  ...
  return (
    <div
      className={cn(
        "relative flex h-[620px] w-full flex-col items-center justify-center overflow-hidden rounded-[32px] border border-[#D7E2EA]/15 bg-black",
        className,
      )}
      ref={containerRef}
